...
const canvas=document.querySelector('#network'); const ctx=canvas.getContext('2d'); let nodes=[],canvasActive=true,canvasFrame=0,lastCanvasPaint=0; function size(){const d=Math.min(devicePixelRatio,1.25);canvas.width=canvas.clientWidth*d;canvas.height=canvas.clientHeight*d;ctx.setTransform(d,0,0,d,0,0);nodes=Array.from({length:matchMedia('(max-width:700px)').matches?14:26},()=>({x:Math.random()*canvas.clientWidth,y:Math.random()*canvas.clientHeight,vx:(Math.random()-.5)*.14,vy:(Math.random()-.5)*.14}));} function draw(now=0){canvasFrame=0;if(matchMedia('(prefers-reduced-motion: reduce)').matches||!canvasActive||document.hidden)return;if(now-lastCanvasPaint<32){canvasFrame=requestAnimationFrame(draw);return;}lastCanvasPaint=now;ctx.clearRect(0,0,canvas.clientWidth,canvas.clientHeight);nodes.forEach((n,i)=>{n.x+=n.vx;n.y+=n.vy;if(n.x<0||n.x>canvas.clientWidth)n.vx*=-1;if(n.y<0||n.y>canvas.clientHeight)n.vy*=-1;for(let j=i+1;j<nodes.length;j++){const m=nodes[j],d=Math.hypot(n.x-m.x,n.y-m.y);if(d<150){ctx.strokeStyle=`rgba(75,180,255,${(1-d/150)*.16})`;ctx.beginPath();ctx.moveTo(n.x,n.y);ctx.lineTo(m.x,m.y);ctx.stroke();}}ctx.fillStyle='rgba(111,215,255,.5)';ctx.beginPath();ctx.arc(n.x,n.y,1.2,0,7);ctx.fill();});canvasFrame=requestAnimationFrame(draw);} function updateCanvas(active=canvasActive){canvasActive=active;if(canvasActive&&!document.hidden&&!canvasFrame)canvasFrame=requestAnimationFrame(draw);else if((!canvasActive||document.hidden)&&canvasFrame){cancelAnimationFrame(canvasFrame);canvasFrame=0;}} size();addEventListener('resize',size);new IntersectionObserver(([entry])=>updateCanvas(entry.isIntersecting)).observe(canvas);document.addEventListener('visibilitychange',()=>updateCanvas());canvasFrame=requestAnimationFrame(draw);
...
function updateScroll(){const max=document.documentElement.scrollHeight-innerHeight;const ratio=max>0?scrollY/max:0;progress.style.transform=`scaleX(${ratio})`;scrollQueued=false;}
...
if(finePointer&&!reducedMotion){
  ...
  let heroFrame=0,heroEvent;document.querySelector('.hero')?.addEventListener('pointermove',e=>{heroEvent=e;if(heroFrame)return;heroFrame=requestAnimationFrame(()=>{const x=(heroEvent.clientX/innerWidth-.5),y=(heroEvent.clientY/innerHeight-.5);heroContent.style.setProperty('--hero-x',`${x*11}px`);heroContent.style.setProperty('--hero-y',`${y*8}px`);heroFrame=0;});},{passive:true});
}
function activateMagnetics(root=document){
  ...
  root.querySelectorAll('.button:not([data-magnetic]),.filters button:not([data-magnetic]),.header-cta:not([data-magnetic])').forEach(el=>{
    ...
    let rect,frame=0,event;el.addEventListener('pointerenter',()=>{rect=el.getBoundingClientRect();},{passive:true});el.addEventListener('pointermove',e=>{event=e;if(frame)return;frame=requestAnimationFrame(()=>{const r=rect||el.getBoundingClientRect();el.style.setProperty('--mag-x',`${(event.clientX-r.left-r.width/2)*.13}px`);el.style.setProperty('--mag-y',`${(event.clientY-r.top-r.height/2)*.18}px`);frame=0;});},{passive:true});
...
function activateCards(){
  document.querySelectorAll('.project-card:not([data-reactive])').forEach(card=>{
    ...
    if(!finePointer||reducedMotion)return;
    let rect,frame=0,event;card.addEventListener('pointerenter',()=>{rect=card.getBoundingClientRect();},{passive:true});card.addEventListener('pointermove',e=>{event=e;if(frame)return;frame=requestAnimationFrame(()=>{const r=rect||card.getBoundingClientRect(),x=(event.clientX-r.left)/r.width,y=(event.clientY-r.top)/r.height;card.style.setProperty('--card-x',`${x*100}%`);card.style.setProperty('--card-y',`${y*100}%`);card.style.setProperty('--tilt-y',`${(x-.5)*5}deg`);card.style.setProperty('--tilt-x',`${(.5-y)*4}deg`);frame=0;});},{passive:true});
